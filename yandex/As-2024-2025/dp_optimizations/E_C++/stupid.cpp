...
void stupid_solve() {
    ...
    for (int i = 0; i < m; ++i) {
        ll mans = inf;
        for (int j = 0; j < n; ++j) {
            if (binary_search(clients[i].banned_pizzerias.begin(), clients[i].banned_pizzerias.end(), j)) continue;
            mans = std::min<ll>(
                mans, 1ll * (pizzerias[j].s - clients[i].c) * (pizzerias[j].s - clients[i].c) + pizzerias[j].p);
        }
        cout << mans << "\n";
    }
}
...
//     }
//     ST::build();
//     sort(begin(clients), next(begin(clients), n), [](const Client &a, const Client &b) {
//         return a.c < b.c;
//     });
//     for (int i = 0; i < m; ++i) {
//         ll cans = inf;
