...
int main() {
    ...
    for (int32_t i = 0; i < n; ++i) {
        int32_t pp = distance(dp.begin(), lower_bound(dp.begin(), dp.end(), nums[i]));
    ...
    vector<int32_t> res;
    res.reserve(len);
    for (int32_t p = pos[len]; p >= 0; p = prev[p]) res.push_back(nums[p]);
    cout << len << "\n";
    for (auto &e: ranges::reverse_view(res)) cout << e << " ";
    return 0;
}
