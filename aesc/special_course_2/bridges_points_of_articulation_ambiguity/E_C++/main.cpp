...
int main() {
    ...
    std::vector<std::uint32_t> out_edges_cnt(last_used_color + 1, 0);
    std::vector<std::uint32_t> in_cnt(last_used_color + 1, 0);
    for (std::uint32_t i = 0; i < n; ++i) {
        ++in_cnt[colors[i]];
        for (const auto &edge: g[i]) if (colors[edge.u] != colors[i]) ++out_edges_cnt[colors[i]];
    }
    std::uint64_t cnt_res = 0, res = 1, mod = 1e9 + 7;
    for (std::uint32_t i = 1; i <= last_used_color; ++i) {
        if (out_edges_cnt[i] > 1) continue;
        ++cnt_res;
        res = (res * in_cnt[i]) % mod;
    }
    std::cout << cnt_res << " " << res;
    return 0;
}