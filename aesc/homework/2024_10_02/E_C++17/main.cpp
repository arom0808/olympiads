...
int main() {
    ...
    auto time_check_f = [start_t]() {
        int32_t ms_time = chrono::duration_cast<chrono::milliseconds>(get_current_time() - start_t).count();
        return pair<int32_t, bool>{ms_time, ms_time < 4700};
    };
    auto temp_on_time_f = [](int32_t t) {
        return static_cast<long double>(t) / 4700;
    };
    Annealing(time_check_f, temp_on_time_f);
    for (int32_t i = 0; i < n; ++i) {
        cout << best_path.seq[i] + 1 << " ";
    }
    // cout << "my res: " << best_path.props.len << "\n";
    // Path jp;
    // jp.seq = {2, 1, 3, 4, 0};
    // jp.upd_props();
    // cout << "jury res: " << jp.props.len << "\n";
    return 0;
}
